...
class PipTool:
    ...
    def __init__(self, python_path: str = sys.executable):
        ...
        # 初始化时自动安装必要依赖
        try:
            self._run_command(["pip", "install", "--upgrade", "setuptools", "wheel"], pip=False)
        except subprocess.CalledProcessError as exc:
            raise PipManagerException("基础依赖安装失败") from exc

    def _run_command(self, args: List[str], capture_output: bool = True, pip: bool = True) -> subprocess.CompletedProcess:
        """执行系统命令的基础方法
