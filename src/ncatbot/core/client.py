...
class BotClient:
    ...
    def __init__(self, plugins_path="plugins", *args, **kwargs):
        ...
        def check_duplicate_register():
            if BotClient.registered:
                ERROR_DUPLICATED = "一个主程序只允许注册一个 BotClient 实例, 请检查你的引导程序"
    ...
    def run_blocking(self, *args, **kwargs):
        ...
        lock = Lock()
        lock.acquire()  # 全局加锁
        self.add_startup_handler(lambda: lock.release())  # 在 NcatBot 接口可用时, 释放全局锁
    ...
    async def handle_private_event(self, msg: dict):
        ...
        await self.plugin_sys.event_bus.publish_async(Event(OFFICIAL_PRIVATE_MESSAGE_EVENT, msg, EventSource(msg.user_id, "root")))
    ...
    async def handle_request_event(self, msg: dict):
        ...
        await self.plugin_sys.event_bus.publish_async(Event(OFFICIAL_REQUEST_EVENT, msg))
    ...
    async def handle_startup_event(self):
        ...
        await self.plugin_sys.event_bus.publish_async(Event(OFFICIAL_STARTUP_EVENT, None))
    ...
    async def handle_shutdown_event(self):
        ...
        await self.plugin_sys.event_bus.publish_async(Event(OFFICIAL_SHUTDOWN_EVENT, None))
    ...
    async def handle_heartbeat_event(self):
        ...
        await self.plugin_sys.event_bus.publish_async(Event(OFFICIAL_HEARTBEAT_EVENT, None))

    async def _run_async(self):
        def info_subscribe_message_types():
            subsribe_group_message_types = (
                "全部消息类型" if self._subscribe_group_message_types is None else self._subscribe_group_message_types
            )
            subsribe_private_message_types = (
                "全部消息类型" if self._subscribe_private_message_types is None else self._subscribe_private_message_types
            )
